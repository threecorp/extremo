...
class ReservePage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    useAutomaticKeepAlive(); // TODO(unimpl): re-rebuild this page when switching tabs

    final booksState = useState<List<BookModel>>([]);

    Future<void> fetchBooks(DateTime openedAt, DateTime closedAt) async {
      booksState.value = await ref.read(filterBooksCaseProvider(openedAt, closedAt).future);
    }
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: Text(clientsState.value.firstOrNull != null ? 'ID:${clientsState.value.first.pk} User' : 'Select User'),
          ),

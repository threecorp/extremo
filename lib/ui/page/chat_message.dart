...
class ChatMessagePage extends StatelessWidget {
  const ChatMessagePage({required this.id, super.key});

  final int id;

  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(), body: _ChatMessagePage(id: id, key: super.key));
}

class _ChatMessagePage extends HookConsumerWidget {
  const _ChatMessagePage({required this.id, super.key});
  final int id; // toFk
...
//   @override
//   Widget build(BuildContext context, WidgetRef ref) {
//     final messagesProvider = ref.watch(listPagerChatMessagesCaseProvider);
//     final messagesNotifier = ref.watch(listPagerChatMessagesCaseProvider.notifier);
//
...
//             List.from(messagesNotifier.state.value!)..[index] = updatedChatMessageModel,
//           );
//
